...
const Feedback = () => {
    ...
    const addfeed = useFormik({
        ...
        onSubmit: (values, { resetForm }) => {
            ...
            axios.post('http://localhost:5000/feed/add/', values, { headers: { 'x-auth-token': token } })
                .then((result) => {
    ...
    return (
        <div className='h-screen flex justify-center items-center w-screen'>
            <form onSubmit={addfeed.handleSubmit}>
                <div className="bg-slate-800 border border-slate-700 grid grid-cols-6 gap-2 rounded-xl p-2 text-sm">
                    ...
                    <textarea
                        id='feedback'
                        value={addfeed.values.feedback}
                        onChange={addfeed.handleChange}
                        className="bg-slate-700 text-slate-300 h-28 placeholder:text-slate-300 placeholder:opacity-50 border border-slate-600 col-span-6 resize-none outline-none rounded-lg p-2 duration-300 focus:border-slate-300" placeholder="Your feedback..." defaultValue={""} />
                    ...
                    <button type='submit' className="col-span-2 stroke-slate-300 bg-slate-700 focus:stroke-blue-200 focus:bg-blue-600 border border-slate-600 hover:border-slate-300 rounded-lg p-2 duration-300 flex justify-center items-center">
